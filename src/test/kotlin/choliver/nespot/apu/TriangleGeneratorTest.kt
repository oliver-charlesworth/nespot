...
class TriangleGeneratorTest {
  private val gen = TriangleGenerator(cyclesPerSample = 4.toRational()).apply {
  ...
  fun `linear counter`() {
    gen.linear = 40
    val seq = gen.take(64, Ticks(quarter = 1, half = 0))
  ...
  fun `setting length counter reloads linear counter`() {
    gen.linear = 40
    val seq1 = gen.take(4, Ticks(quarter = 1, half = 0))
    gen.length = 5 // Whatever
    val seq2 = gen.take(60, Ticks(quarter = 1, half = 0))
