...
class TriangleSynthTest {
  ...
  fun `linear counter`() {
    synth.linear = 40
    val seq = synth.take(64, Ticks(quarter = 1, half = 0))
  ...
  fun `setting length counter reloads linear counter`() {
    synth.linear = 40
    val seq1 = synth.take(4, Ticks(quarter = 1, half = 0))
    synth.length = 5 // Whatever
    val seq2 = synth.take(60, Ticks(quarter = 1, half = 0))
  ...
  fun `length counter`() {
    synth.length = 40
    val seq = synth.take(64, Ticks(quarter = 0, half = 1))
